	...
	return {
		initialize : function(divName, o) {
			...
			if (!GBrowserIsCompatible()) {
				alert("Sorry, this site cannot run on your browser.");
				return;
			}
			...
			this.adsManager = new google.maps.AdsManager(this.map,
					'ca-pub-7898295704528692', {
						maxAdsOnMap : 5
					});
			...
			google.maps.Event.addListener(this.map, 'click', function(overlay,
				...
				if (overlay)
					Map.map.zoomIn(overlaylatlng, true, true);
				if (overlay == null)
		...
		addMarker : function(o) {
			...
			var marker;
			if (o.point == null)
				marker = new google.maps.Marker(new google.maps.LatLng(o.lat,
						o.lon), {
					icon : Map.icon
				});
			else
				marker = new google.maps.Marker(o.point, {
